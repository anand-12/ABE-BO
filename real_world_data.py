...
def bayesian_optimization(args, X_feature, y):
    num_iterations = 100
    initial_points = int(0.1 * num_iterations)
    
    # Convert numpy arrays to torch tensors
    X_feature = torch.tensor(X_feature, dtype=dtype, device=device)
    y = torch.tensor(y, dtype=dtype, device=device).unsqueeze(-1)
    
    # Calculate bounds
    bounds = torch.stack([torch.min(X_feature, dim=0)[0], torch.max(X_feature, dim=0)[0]])

    # Draw initial points
    train_indices = torch.randperm(len(X_feature))[:initial_points]
    train_X = X_feature[train_indices]
    ...
    for i in range(num_iterations):
        ...
        # Find the closest point in X_feature to new_candidates
        distances = torch.cdist(new_candidates, X_feature)
        closest_index = torch.argmin(distances)
        new_X = X_feature[closest_index].unsqueeze(0)
        new_Y = y[closest_index].unsqueeze(0)
    ...
    return max_values, chosen_acq_functions, kernel_names

def run_experiments(args, X_feature, y):
    all_results = []
    ...
    for seed in range(args.seed, args.seed+args.experiments):
        ...
        max_values, chosen_acq_functions, kernel_names = bayesian_optimization(args, X_feature, y)
        end_time = time.time()

        experiment_time = end_time - start_time
        all_results.append([max_values, experiment_time, chosen_acq_functions, kernel_names])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='BoTorch Bayesian Optimization on Real Dataset')
    ...
    # Process the dataset
    ds_grouped = raw_dataset.groupby(feature_name)[objective_name].agg(lambda x: x.unique().mean())
    ds_grouped = (ds_grouped.to_frame()).reset_index()

    X_feature = ds_grouped[feature_name].values
    y = np.array(ds_grouped[objective_name].values)  # Negative because we want to maximize

    # Run experiments
    all_results = run_experiments(args, X_feature, y)

    # Convert to numpy array and save
    all_results_np = np.array(all_results, dtype=object)
    os.makedirs(f"./Results_real_data", exist_ok=True)
    ...
    np.save(f"./Results_real_data/{dataset_name}_{filename}", all_results_np)

    print(f"Results saved to ./Results_real_data/{dataset_name}_{filename}")